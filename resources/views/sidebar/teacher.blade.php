    ...
    <div class="sidebar">
        <div class="user-panel d-flex align-items-center p-3">
            ...
            <div class="info ms-3">
                <a href="#" class="d-block fw-bold" style="font-size: 1rem;">
                    {{ auth()->user()->teacher ? auth()->user()->teacher->full_name_with_extension : auth()->user()->username }}
                </a>
        ...
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column nav-collapse-hide-child nav-child-indent"
                ...
                <li class="nav-item">
                    <a href="{{ route('teacher.dashboard') }}" class="nav-link @yield('active-dashboard')">
                ...
                @php
                    ...
                    $isAdviser = App\Models\Adviser::where('teacher_id', $teacher->id)->exists();
                @endphp
                @if ($isAdviser)
                    <li class="nav-item">
                        <a href="{{ route('teacher.advisory') }}" class="nav-link @yield('active-advisory')">
                ...
                @endif
                <li class="nav-item">
                    <a href="{{ route('teacher.subjects') }}" class="nav-link @yield('active-subjects')">
                        <i class="nav-icon ion ion-clipboard"></i>
                        <p>Subject Handle</p>
                    </a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('teacher.attendance') }}" class="nav-link @yield('active-attendance')">
                        <i class="nav-icon ion ion-calendar"></i>
                        <p>Attendance</p>
                    </a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('teacher.class-records') }}" class="nav-link @yield('active-class-records')">
                        <i class="nav-icon ion ion-ios-book"></i>
                        <p>Class Records</p>
                    </a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('teacher.report-card') }}" class="nav-link @yield('active-report-card')">
