        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-lg-3 my-body mg-b-20 mg-lr-20" style="display: block;">
                    ...
                    <img src="{{ $teacher->getFirstMediaUrl('avatar', 'thumb') ?: asset('dist/img/avatar4.png') }}"
                    ...
                    <div class="row">
                        <div class="form-group col-md-12 col-lg-3">
                            ...
                            <input type="text" class="form-control" id="first_name" name="first_name"
                                value="{{ old('first_name', $teacher->first_name) }}" placeholder="First Name" required>
                        </div>
                        <div class="form-group col-md-12 col-lg-3">
                            <label for="middle_name">Middle Name: <span style="color:red;"></span></label>
                            <input type="text" class="form-control" id="middle_name" name="middle_name"
                                value="{{ old('middle_name', $teacher->middle_name) }}" placeholder="Middle Name">
                        </div>
                        <div class="form-group col-md-12 col-lg-3">
                            <label for="last_name">Last Name: <span style="color:red;">*</span></label>
                            <input type="text" class="form-control" id="last_name" name="last_name"
                                value="{{ old('last_name', $teacher->last_name) }}" placeholder="Last Name" required>
                        </div>
                        <div class="form-group col-md-12 col-lg-3">
                            ...
                            <input type="text" class="form-control" id="extension_name" name="extension_name"
                                value="{{ old('extension_name', $teacher->extension_name) }}" placeholder="Extension Name">
                        </div>
                        <div class="col-lg-12">
                            ...
                            <select class="form-control" id="province_code" name="province_code" required>
                                ...
                                @foreach ($provinces as $province)
                                    <option value="{{ $province->province_code }}"
                                        {{ old('province_code', $teacher->province_code) == $province->province_code ? 'selected' : '' }}>
                            ...
                            <select class="form-control" id="municipality_code" name="municipality_code" required>
                                ...
                                @foreach ($municipalities as $municipality)
                                    <option value="{{ $municipality->municipality_code }}"
                                        {{ old('municipality_code', $teacher->municipality_code) == $municipality->municipality_code ? 'selected' : '' }}>
                            ...
                            <select class="form-control" id="barangay_code" name="barangay_code" required>
                                ...
                                @foreach ($barangays as $barangay)
                                    <option value="{{ $barangay->barangay_code }}"
                                        {{ old('barangay_code', $teacher->barangay_code) == $barangay->barangay_code ? 'selected' : '' }}>
                        ...
                        <div class="form-group col-md-12 col-lg-3">
                            ...
                            <input type="text" class="form-control" id="zip_code" name="zip_code"
                                value="{{ old('zip_code', $teacher->zip_code) }}" placeholder="Zip Code" required
                                readonly>
                        </div>
                        <div class="col-lg-12">
                            ...
                            <select id="religion" name="religion" class="form-control">
                                ...
                                @foreach (['Roman Catholic', 'Seventh-Day Adventist', 'Iglesia ni Cristo', 'Jehovah Witnesses', 'Pentecostal', 'Church of Christ', 'Christian', 'Baptist', 'Protestant', 'Born Again', 'Muslim'] as $religion)
                                    <option value="{{ $religion }}"
                                        {{ old('religion', $teacher->religion) == $religion ? 'selected' : '' }}>
                                        {{ $religion }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-md-12 col-lg-3">
                            ...
                            <input type="date" class="form-control" id="birthday" name="birthday"
                                value="{{ old('birthday', $teacher->birthday) }}" required>
                        </div>
                        <div class="form-group col-md-12 col-lg-3">
                            ...
                            <select type="text" class="form-control" id="sex" name="sex">
                                ...
                                <option value="Male" {{ old('sex', $teacher->sex) == 'Male' ? 'selected' : '' }}>Male
                                </option>
                                <option value="Female" {{ old('sex', $teacher->sex) == 'Female' ? 'selected' : '' }}>
                                    Female</option>
                            </select>
                        </div>
                        <div class="form-group col-md-12 col-lg-3">
                            ...
                            <select class="form-control" id="civil_status" name="civil_status">
                                ...
                                <option value="Single"
                                    {{ old('civil_status', $teacher->civil_status) == 'Single' ? 'selected' : '' }}>Single
                                </option>
                                <option value="Married"
                                    {{ old('civil_status', $teacher->civil_status) == 'Married' ? 'selected' : '' }}>
                                    Married</option>
                                <option value="Widowed"
                                    {{ old('civil_status', $teacher->civil_status) == 'Widowed' ? 'selected' : '' }}>
                                    Widowed</option>
                                <option value="Divorced"
                                    {{ old('civil_status', $teacher->civil_status) == 'Divorced' ? 'selected' : '' }}>
                                    Divorced</option>
                            </select>
                        </div>
                        <div class="form-group col-md-12 col-lg-6">
                            ...
                            <input type="email" class="form-control" id="email" name="email"
                                value="{{ old('email', $teacher->email) }}" placeholder="Email Address" required>
                        </div>
                        <div class="form-group col-md-12 col-lg-6">
                            <label for="contact">Contact Number: <span style="color:red;">*</span></label>
                            <input type="text" class="form-control" id="contact" name="contact"
                                value="{{ old('contact', $teacher->contact) }}" data-mask="(+63) 999-999-9999"
        ...
        $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#province_code').val('{{ $teacher->province_code }}').trigger('chosen:updated');
            ...
            $('#province_code').change(function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#municipality_code').val('{{ $teacher->municipality_code }}')
            ...
            $('#municipality_code').change(function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#barangay_code').val('{{ $teacher->barangay_code }}').trigger(
                            'chosen:updated');
            ...
            $('#extension_name').val('{{ $teacher->extension_name ?? '' }}');
            $('#religion').val('{{ $teacher->religion }}');
            $('#birthday').val('{{ $teacher->birthday->format('Y-m-d') }}');
            $('#sex').val('{{ $teacher->sex }}');
            $('#civil_status').val('{{ $teacher->civil_status }}');
            $('#email').val('{{ $teacher->email }}');
            $('#contact').val('{{ $teacher->contact }}');
            $('#user_picture').attr('src',
                '{{ $teacher->getFirstMediaUrl('avatar', 'thumb') ?: asset('dist/img/avatar4.png') }}');

            selectedBrgy = '{{ $teacher->barangay_code }}';
            selectedMunicipality = '{{ $teacher->municipality_code }}';
                ...
                if (isEmptyField) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                method: 'POST',
                                url: `/teachers/updateImage/{{ $teacher->id }}`,
                                data: formData,
                                processData: false,
                                contentType: false,
                                dataType: 'JSON',
